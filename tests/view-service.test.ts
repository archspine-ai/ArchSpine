...
import {
  ...
  GENERATOR_VERSION,
  type SpineFolderUnit,
  type SpineProjectUnit,
  type SpineUnit,
...
function writeProjectIndex(rootDir: string, project: SpineProjectUnit): void {
  const projectPath = path.join(rootDir, '.spine', 'index', 'project.json');
  fs.mkdirSync(path.dirname(projectPath), { recursive: true });
  fs.writeFileSync(projectPath, JSON.stringify(project, null, 2));
}

function writeFolderIndex(rootDir: string, directory: string, folder: SpineFolderUnit): void {
  const folderPath = path.join(rootDir, '.spine', 'index', directory, 'folder.json');
  fs.mkdirSync(path.dirname(folderPath), { recursive: true });
  fs.writeFileSync(folderPath, JSON.stringify(folder, null, 2));
}
...
describe('view service', () => {
  ...
  it('derives an architecture diagram during the dedicated view step', async () => {
    writeProjectIndex(testDir, {
      schemaVersion: CURRENT_SCHEMA_VERSION,
      projectName: 'archspine-view-service',
      role: 'Architectural governance toolchain',
      responsibility: 'Indexes repositories and renders machine-readable and human-readable views.',
      modules: [
        { directory: 'src/services', role: 'Runtime services', childCount: 4 },
        { directory: 'src/infra', role: 'Infrastructure layer', childCount: 5 },
      ],
      provenance: {
        indexedAt: '2026-04-13T00:00:00.000Z',
        generatorVersion: GENERATOR_VERSION,
    ...
    writeFolderIndex(testDir, 'src/services', {
      schemaVersion: CURRENT_SCHEMA_VERSION,
      directory: 'src/services',
      role: 'Runtime orchestration services',
      responsibility: 'Coordinates sync, check, fix, and view derivation services.',
      children: [
        { filePath: 'src/services/view-service.ts', role: 'View derivation', fileKind: 'source' },
      ],
      provenance: {
        indexedAt: '2026-04-13T00:00:00.000Z',
        generatorVersion: GENERATOR_VERSION,
    ...
    writeFolderIndex(testDir, 'src/infra', {
      schemaVersion: CURRENT_SCHEMA_VERSION,
      directory: 'src/infra',
      role: 'Infra layer',
      responsibility: 'Provides LLM access, output management, and rendering helpers.',
      children: [{ filePath: 'src/infra/output.ts', role: 'Output manager', fileKind: 'source' }],
      provenance: {
        indexedAt: '2026-04-13T00:00:00.000Z',
        generatorVersion: GENERATOR_VERSION,
        pipelineStages: ['aggregate'],
    ...
    const llmClient: LLMClient = {
      ...
      generateFolderSummary: vi.fn(),
      generateProjectSummary: vi.fn(),
      generateText: vi.fn(async () => ({
        content: JSON.stringify({
          title: 'ArchSpine Architecture',
          subtitle: 'Indexes code and renders views.',
          nodes: [
          ...
          summaryCards: [
            ...
            { heading: 'Key Dependencies', points: ['Rules', 'Git', 'Outputs'] },
            {
              ...
              points: ['Repo runtime', 'Protected outputs', 'External Git data'],
            },
          ],
        }),
      })),
    ...
    expect(saveViewHtml).toHaveBeenCalledWith(
  ...
  it('skips architecture diagram generation when the spec is invalid', async () => {
    writeProjectIndex(testDir, {
      schemaVersion: CURRENT_SCHEMA_VERSION,
      projectName: 'archspine-view-service',
      role: 'Architectural governance toolchain',
      responsibility: 'Indexes repositories and renders machine-readable and human-readable views.',
      modules: [{ directory: 'src/services', role: 'Runtime services', childCount: 4 }],
      provenance: {
        indexedAt: '2026-04-13T00:00:00.000Z',
        generatorVersion: GENERATOR_VERSION,
        pipelineStages: ['aggregate'],
    ...
    const llmClient: LLMClient = {
      ...
      generateText: vi.fn(async () => ({
        content: JSON.stringify({
          ...
          subtitle: 'Missing nodes and cards.',
          nodes: [{ id: 'only-one', label: 'Only One', type: 'backend' }],
          edges: [],
          summaryCards: [],
        }),
      })),
