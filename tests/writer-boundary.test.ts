...
describe('spine writer boundary', () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(testDir, '.spine', 'view'), { recursive: true });
    ...
    fs.writeFileSync(path.join(testDir, '.spine', 'view', 'public-surface.json'), '{"ok":true}\n');
  ...
  it('locks protected outputs to readonly mode', () => {
    ...
    expect(isWritable(path.join(testDir, '.spine', 'atlas', 'en-US', 'project.md'))).toBe(false);
    expect(isWritable(path.join(testDir, '.spine', 'view', 'public-surface.json'))).toBe(false);
    expect(isWritable(path.join(testDir, '.spine', 'cache.db'))).toBe(false);
    expect(isWritable(path.join(testDir, '.spine', 'cache.db-wal'))).toBe(false);
    expect(isWritable(path.join(testDir, '.spine', 'cache.db-shm'))).toBe(false);
    expect(isWritable(path.join(testDir, '.spine', '.lock'))).toBe(false);
  });
