...
describe('view command', () => {
  ...
  it('shows default enabled views when no explicit enabledViews config exists', async () => {
    ...
    expect(logs.some((line) => line.includes('Experimental view layer: enabled'))).toBe(true);
  ...
  it('shows no effective views when the layer is disabled', async () => {
    ...
    expect(logs.some((line) => line.includes('Experimental view layer: disabled'))).toBe(true);
