...
describe('init CLI integration', () => {
  ...
  it('repo strategy set migrates local repositories to distributable without rerunning init', () => {
    ...
    const output = execFileSync(
      'node',
      [builtCliPath, 'config', 'repo', 'strategy', 'set', 'distributable'],
      {
        cwd: rootDir,
        encoding: 'utf-8',
        stdio: 'pipe',
  ...
  it('repo strategy set warns but does not untrack snapshot files when downgrading to local', () => {
    ...
    const result = spawnSync('node', [builtCliPath, 'config', 'repo', 'strategy', 'set', 'local'], {
      cwd: rootDir,
      encoding: 'utf-8',
      stdio: 'pipe',
    });
  ...
  it('try reports distributable snapshot posture without mutating repository state', () => {
    ...
    expect(output).toContain('ArchSpine Try');
    expect(output).toContain('ArchSpine adopted: yes');
    expect(output).toContain('--- What This Means ---');
    expect(beforeFiles).toEqual(afterFiles);
  });

  it('try recommends sync when ArchSpine is adopted but distributable snapshot is incomplete', () => {
    const rootDir = makeTempDir();
    createdDirs.push(rootDir);
    ...
    expect(output).toContain('ArchSpine is configured but the semantic snapshot is partial');
    expect(output).toContain('Run `spine sync` to build it');
    expect(output).toContain('--- What This Means ---');
  });
