...
import { resolveEnabledViews, resolveExperimentalViewLayer } from '../src/services/view/index.js';

describe('experimental view layer resolution', () => {
  let testDir: string;
  const originalEnv = process.env.SPINE_EXPERIMENTAL_VIEW_LAYER;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.SPINE_EXPERIMENTAL_VIEW_LAYER = originalEnv;
    }
  ...
  it('defaults to disabled when no project config or environment override exists', () => {
    const resolved = resolveExperimentalViewLayer(new Config(testDir));
  ...
  it('uses default enabled views when enabledViews is unset', () => {
    ...
    expect(resolved.value).toEqual(['public-surface', 'risk-hotspots', 'architecture-diagram']);
