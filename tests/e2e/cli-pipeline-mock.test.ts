...
describe('E2E: Pipeline with mock LLM', () => {
  ...
  describe('spine build', () => {
    it('BLD-01: first build creates index and atlas', () => {
      ...
      // Index and atlas should exist
      expect(fs.existsSync(path.join(dir, '.spine', 'index', 'project.json'))).toBe(true);
      expect(fs.existsSync(path.join(dir, '.spine', 'atlas', 'English', 'project.md'))).toBe(true);
  ...
  describe('spine sync', () => {
    ...
    it('SYNC-02: sync --hook does not generate atlas summaries', () => {
      const dir = setupInitializedProject();
      createdDirs.push(dir);
      ...
      expect(status).toBe(0);
      // Hook mode: index refresh only, no atlas regeneration
      expect(output).toMatch(/index|hook|sync|Processed/i);
    }, 120_000);

    it('SYNC-03: sync --retry-failed gracefully handles no prior failures', () => {
      const dir = setupInitializedProject();
      createdDirs.push(dir);
  ...
  describe('spine publish', () => {
    it('PUB-01: publish after build succeeds', () => {
      const dir = setupInitializedProject();
      createdDirs.push(dir);

      runCliOk(['build'], dir);

      const { stdout, stderr, status } = runCli(['publish'], dir);
      const output = `${stdout}${stderr}`;
  ...
  describe('spine god', () => {
    it('GOD-01: god mode generates project god file', () => {
      const dir = setupInitializedProject();
      createdDirs.push(dir);
      ...
      expect(status).toBe(0);
      const godPath = path.join(dir, '.spine', 'test-god.md');
      const godExists = fs.existsSync(godPath) || output.includes('god');
      expect(godExists).toBe(true);
    }, 120_000);
  });

  describe('spine fix', () => {
    it('FIX-01: fix with no violations reports nothing to fix', () => {
      const dir = setupInitializedProject();
      createdDirs.push(dir);
