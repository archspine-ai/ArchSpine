...
describe('Schema Compliance', () => {
  ...
  it('should accept legacy string invariants for compatibility with existing indexed output', () => {
    const withStringInvariants = {
      schemaVersion: CURRENT_SCHEMA_VERSION,
      identity: {
      ...
      semantic: {
        ...
        responsibilities: ['Configure runtime behavior'],
        outOfScope: [],
        invariants: [
          'Must not contain production credentials',
          'Should keep environment-specific values externalized',
        ],
        changeIntent: {
          architecturalIntent: null,
          recentChangeIntent: null,
    ...
    const valid = validate(withStringInvariants);
    expect(valid).toBe(true);
  });
});
