...
describe('task runtime state management', () => {
  ...
  it('creates a strict validation context with the expected runtime defaults', () => {
    ...
    expect(prepared.context.promptTier).toBe('lite');
    expect(prepared.context.validatePolicy).toBe('strict');
    expect(prepared.context.generationFlow).toBe('semantic-first');
    expect(prepared.context.summarizeConcurrency).toBe(9);
    expect(prepared.context.summarizeRetryLimit).toBe(4);
    expect(prepared.context.targetLocales).toEqual(['zh-CN']);
    ...
    expect(prepared.context.writeAtlasDocs).toBe(true);
  ...
  it('supports no-llm and no-aggregator contexts with summarize defaults', () => {
    ...
    expect(prepared.context.promptTier).toBe('balanced');
    expect(prepared.context.validatePolicy).toBe('default');
    expect(prepared.context.generationFlow).toBe('together');
    expect(prepared.context.summarizeConcurrency).toBe(3);
    expect(prepared.context.summarizeRetryLimit).toBe(2);
    expect(prepared.context.targetLocales).toEqual(['en-US']);
    expect(prepared.context.enabledViews).toEqual([]);
    expect(prepared.context.invalidEnabledViews).toEqual([]);
    expect(prepared.context.writeAtlasDocs).toBe(false);
