...
describe('Config', () => {
  ...
  it('persists config facade mutations across llm, artifact, hook, and init-state settings', () => {
    ...
    expect(reloaded.getHookSyncMode()).toBe('hook');
  ...
  it('supports delegated config accessors and clearing persisted values', () => {
    ...
    config.setSupportedValue('hooks.syncMode', 'hook');
    config.setSupportedValue('artifacts.enabledViews', [
      'public-surface',
      'risk-hotspots',
      'public-surface',
    ]);
    config.setInjectedPackageScripts(['spine:sync', 42, 'spine:sync'] as unknown as string[]);

    expect(config.getSupportedValue('llm.provider')).toBe('openai');
    expect(config.getSupportedValue('hooks.syncMode')).toBe('hook');
    expect(config.getSupportedValue('artifacts.enabledViews')).toEqual([
      'public-surface',
      'risk-hotspots',
    ]);
    expect(config.getInjectedPackageScripts()).toEqual(['spine:sync']);
