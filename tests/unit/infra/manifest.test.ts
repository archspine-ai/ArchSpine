...
describe('Manifest', () => {
  ...
  it('Main Path: should correctly interpret needs initial sync when DB says 0 files', () => {
  ...
  it('Main Path: should not be needs initial sync when sync is complete', () => {
    ...
    const manifest = Manifest.open(tmpDir);
    expect(manifest.needsInitialSync()).toBe(false);
  });
  ...
  it('Boundary/Exception: handle broken manifest file with warnings', () => {
    ...
    const manifest = Manifest.open(tmpDir);
    expect(manifest.needsInitialSync()).toBe(true); // Should fallback to needs initial sync implicitly if parse fails
