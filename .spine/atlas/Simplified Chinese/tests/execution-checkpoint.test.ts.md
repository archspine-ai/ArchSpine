...
# ArchSpine 恢复候选派生测试套件

## 角色
用于验证执行检查点恢复候选派生逻辑的 Vitest 测试套件，涵盖同步和检查操作。
...
- **不涉及：** 超出临时目录范围的实际文件系统操作集成测试；恢复候选派生的性能或负载测试；与恢复候选派生无关的其他执行检查点存储方法的测试。

## 最重要的导出行为
- `describe('resume-services')` — 顶级测试套件分组。
- `it('deriveSyncResumeCandidateFiles')` — 验证同步恢复候选生成。
- `it('deriveCheckResumeCandidateFiles')` — 验证检查恢复候选生成。