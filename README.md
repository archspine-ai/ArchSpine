...
- `Node.js >= 20.18.1` (current LTS recommended; Node 21 is not supported by the native SQLite dependency)
- `spine init` only bootstraps config; run `spine build` for the first semantic mirror
- `spine init` supports Git artifact strategies: `local` (runtime state kept out of Git) and `distributable` (snapshots kept in Git and marked as generated). For non-interactive scenarios, use `spine init --artifact-strategy distributable`
- For full `sync / check / fix` capabilities, run `spine llm setup` after `spine init`

## God Mode

`spine god` generates a single-file semantic archive `.spine/<repo-name>-god.md`—compressing the entire repository state into one Markdown file containing summaries, file lists, per-file responsibilities, dependencies, public interfaces, and drift states. Designed for human review, demos, and experiments. Non-production path; data sourced from current `.spine/index/` state, overwrites previous output on each run.

## MCP Integration

ArchSpine can expose the local `.spine` control plane over a STDIO MCP server (read-only semantic plane; does not write official `.spine` artifacts):
...
Agents can read `spine://project`, `spine://folder/{path}`, `spine://file/{filePath}`, and call capabilities including `spine_query_invariants`, `spine_query_responsibilities`, `spine_preview_scan`, `spine_get_drift_history`, and more. Full guide: [docs/how-to/MCP.md](./docs/how-to/MCP.md).

## CLI Capabilities

Commands grouped by function. Full guide: [RUNBOOK](./docs/how-to/RUNBOOK.md).
...
| `spine remove --yes`                    | Skip confirmation prompt                              |
| `spine llm setup`                       | Configure LLM providers                               |
| `spine languages show`                  | View current output language                          |
| `spine languages set`                   | Set output language                                   |
| `spine repo check`                      | Detect drift between config and managed files         |
| `spine repo strategy set <mode>`        | Migrate artifact strategy (`local` ↔ `distributable`) |
...
| `spine sync --retry-failed`      | Retry previously failed sync items           |
| `spine sync --repair-violations` | Repair externally edited protected artifacts |
...
| `spine info`           | View repo config and statistics          |
...
| Command                     | Description                       |
| --------------------------- | --------------------------------- |
| `spine hook on`             | Install managed pre-commit hook   |
| `spine hook off`            | Uninstall managed pre-commit hook |
...
| Command           | Description            |
| ----------------- | ---------------------- |
| `spine mcp start` | Start MCP STDIO server |

**Experimental**

| Command      | Description                                      |
| ------------ | ------------------------------------------------ |
| `spine god`  | Generate single-file semantic archive (Markdown) |
| `spine view` | Preview architecture and risk diagnostic view    |
...
Read [CONTRIBUTING.md](./CONTRIBUTING.md) before changing runtime, protocol, or docs. Participation is governed by our [Code of Conduct](./CODE_OF_CONDUCT.md).
