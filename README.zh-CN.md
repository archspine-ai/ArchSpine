...
当前仓库 `src/` 目录代码规模约 200+ 文件、2 万+ 行。运行全仓重建指令 `spine build`，同时生成英文和简体中文两套架构文档，全程耗时约 `[待补充]` 分钟，消耗约 `[待补充]` 万 Input Token 和 `[待补充]` 万 Output Token（以 DeepSeek 计费约 `[待补充]` 元人民币）。这笔带来长效架构安全的语义投资，背后是深思熟虑的架构设计与极端的边界性能优化。
...
MCP 模式下可读取 `spine://project`、`spine://folder/{path}`、`spine://file/{filePath}`，以及调用 `spine_query_invariants`、`spine_query_responsibilities`、`spine_preview_scan`、`spine_get_drift_history` 等关键能力。完整说明见 [docs/integrations/mcp.md](./docs/integrations/mcp.md)。
...
- [运行手册](./docs/zh-CN/guides/RUNBOOK.md)
- [God Mode 指南](./docs/zh-CN/guides/GOD-MODE.md)
- [MCP 集成](./docs/zh-CN/integrations/mcp.md)
- [Demo 演示](./docs/zh-CN/examples/demo.md)
- [协议规范 v1.0.0](./docs/specs/PROTOCOL.md)
- [文档清单](./docs/README.md)
