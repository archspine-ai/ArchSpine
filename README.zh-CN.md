...
## 14.89M Tokens，仅需 $0.17 的极致实测

ArchSpine 仓库已实现自举——我们用它治理了自己，并成功发现并消灭了早期的 god file。

当前仓库 `src/` 目录代码规模约 200+ 文件、2 万+ 行。运行全仓重建指令 `spine build`，全程耗时约 7.5 分钟，消耗 **14.89M** Input Token 和 2.47M Output Token，以 DeepSeek 计费仅需 **$0.17**。这笔极其低廉的长效架构安全投资，背后是深思熟虑的架构设计与极端的边界性能优化。

（运行表现可直接查看本仓库的 `.spine/atlas/` 结构镜像和 `.spine/view/` 宏观总结。）

## 我为什么要做 ArchSpine

**作为学生，想理解代码库**
我学了八年编程，一直想彻底看懂代码库——AI 让它成为现实。但在 ArchSpine 之前，我只能手动复制获取解析，时常丢失上下文。于是我开始思考：能不能一次性获得全景性的语义？

**作为 Vibe Coder，想掌控 Vibe 代码**
2025 年我用 vibe coding 开发项目，第一次放任 Agent "先跑，再审"。我逐渐无法掌控代码库的"增殖"——跨层调用、乱造接口。我开始思考：能不能和 Agent 一起，共同阅读一个记录语义变更的可控文件夹？

**作为 Coding Agent** _（这段话由模型编写）_
面对庞大的陌生仓库，我要去哪找依赖？该不该修改底层库？这以前全靠瞎猜。有了 ArchSpine，我直接拉取本地 `.spine/` 控制面——稳定的全局视野、模块边界、底线约束。我不容易跑偏，也能更准确地理解人类意图。

**作为不懂编程的 PM（或 Tech Lead）**
我不用看懂代码逻辑，但我能看懂 `.spine/` 生成的架构与职责映射！通过定期的 `spine status` 和 `spine history` 审计，只要检查出 0 违规，且核心职责没有偏离，我就知道架构依然健康——我可以用这套实体产物来验证团队是否在正确前行。

## AI 编程的技术债危机

AI 写代码越来越快，但一个隐性问题正在蔓延——**不可见的技术债**。

当你放任 Agent "先跑，再审"，它会跨层调用、乱造接口、打破模块边界。这些破坏不一定是 bug，它们是架构层面的漂移：单次看起来无害，累积起来就让代码库不可持续。根源在于：**Agent 缺少稳定的架构上下文**——它不知道每个模块的职责边界在哪，也不清楚哪些依赖是禁止的。

传统的解决方案（提示词文件、RAG）能缓解问题，但它们是"建议"。Agent 读了之后仍然可以忽略。

ArchSpine 的回答是：**在本地建立一套实体的语义基线（Semantic Baseline）**。这是一个结构化的、机器可验证的控制面。Agent 在动手之前能读职责边界和红线规则；人类在合并之前能审计架构漂移；整个仓库的语义状态被持续追踪和版本化。
...
下面这张图说明 ArchSpine 在 AI 协作栈中的位置：它深度融入"结构化代码库表达"和"工程工作流"的那一层。
...
- `Node.js >= 20.18.1`（推荐当前 LTS；原生 SQLite 依赖不支持 Node 21）
...
所有命令按功能分组，完整运行说明见 [运行手册](./docs/zh-CN/how-to/RUNBOOK.md)。
