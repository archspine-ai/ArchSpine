...
为了保护 LLM 上下文窗口和系统内存，ArchSpine 对总结任务强制执行 **2MB 的文件大小上限**。
如果被跟踪的源文件超过 2MB，`spine sync` 将刻意抛出错误并中止执行。此举旨在强制用户将超大文件加入 `.spineignore` 或 `.gitignore` 中。
大于 2MB 的文件通常是生成产物、大型数据集或打包后的代码，它们对架构治理没有任何语义价值，理应从语义镜像中排除。
