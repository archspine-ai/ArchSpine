...
ArchSpine 托管的 `.gitignore` block 会始终忽略本地 fallback 凭证文件：

- `.spine/secrets.json`
- `secrets.json`

这些文件只在系统凭证后端无法验证 secret 写入时作为兜底存在。它们属于本地运行态，不属于可分发产物，也不应该进入 Git。
...
- 如果项目 fallback secret 被写入 `.spine/secrets.json`，但仓库 `.gitignore` 已不再忽略它，ArchSpine 会打印高优先级告警，并指向 `spine repo strategy set <local|distributable>` 或手动修复 `.gitignore`
...
如果你要接本地模型，请看 [本地 LLM 指南](LOCAL-LLM)。
