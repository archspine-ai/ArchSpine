...
export function checkRepositoryStrategy(
  ...
  if (strategy === 'local') {
    ...
    } else if (!managedBlockIncludesAll(gitignoreBlock, ['.spine/index/'])) {
      issues.push(
        '.gitignore managed block does not ignore .spine/index/ (expected for local strategy).',
      );
    }
    ...
    if (gitignoreBlock !== null && gitignoreBlock.includes('.spine/index/')) {
      issues.push(
        '.gitignore managed block ignores .spine/index/, but strategy is distributable. ' +
...
export interface V2BlockSyncResults {
  v2Briefing: V2BlockSyncStatus;
  mcpTools: V2BlockSyncStatus;
  health: V2BlockSyncStatus;
}

export function syncV2AgentInstructionBlocks(
  rootDir: string,
  selectedAgentInstructionsFile: string,
): V2BlockSyncResults {
  const v2Briefing = syncV2BriefingBlock(rootDir, selectedAgentInstructionsFile).status;
  const mcpTools = syncMCPToolsBlock(rootDir, selectedAgentInstructionsFile).status;
  const health = syncHealthBlock(rootDir, selectedAgentInstructionsFile).status;
  return { v2Briefing, mcpTools, health };
}
...
export function syncAgentInstructionsConfiguration(
  ...
  // Inject V2 briefing, MCP tools, and health blocks if the corresponding view artifacts exist.
  syncV2AgentInstructionBlocks(rootDir, selectedAgentInstructionsFile);
