import type { ArchDiagramSpec, ArchDiagramNode, ArchNodeType } from '../../types/view.js';
...
function groupByLayer(nodes: ArchDiagramNode[]): ArchDiagramNode[][] {
  return LAYER_ORDER.map((type) => nodes.filter((node) => node.type === type)).filter(
    (group) => group.length > 0,
  );
}

function layoutNodes(nodes: ArchDiagramNode[]): {
...
function renderEmptyState(spec: ArchDiagramSpec): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
export class ArchitectureDiagramRenderer {
  public static render(spec: ArchDiagramSpec): string {
...
<body>
  <main>
    <section class="frame">
      <div class="eyebrow">Experimental View</div>
      <h1>${escapeHtml(spec.title)}</h1>
