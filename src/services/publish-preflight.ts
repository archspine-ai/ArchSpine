...
export function assertPublishRuntimeBaseline(rootDir: string): void {
  ...
  if (!exists(spineDir)) {
    throw new ArchSpineError(
      ...
      "[Spine] .spine runtime layer is missing. Run 'spine init' and 'spine build' before publishing.",
  ...
  if (exists(lockPath)) {
    ...
    try {
      ...
      throw new ArchSpineError(
        ...
        '[Spine] Runtime lock file is invalid or uses an unsupported format (.spine/.lock). Confirm no ArchSpine process is still running, then remove the lock and rebuild runtime state if needed.',
        {
          cause: error,
          context: {
            lockPath,
    ...
    if (ownerKnown && ownerPid !== null) {
      try {
        ...
        if (sysErr.code === 'ESRCH') {
          throw new ArchSpineError(
            ErrorCodes.PublishLockActive,
            "[Spine] Found a stale runtime lock (.spine/.lock). Run 'spine build' to rebuild runtime state before publishing.",
            {
              context: {
                lockPath,
                ownerPid,
        ...
        if (sysErr.code === 'EPERM') {
          throw new ArchSpineError(
            ErrorCodes.PublishLockActive,
            '[Spine] Runtime lock owner could not be verified (.spine/.lock). Publish fails closed when lock ownership is unverifiable; clear the lock only after confirming no ArchSpine process is still running.',
            {
              context: {
                lockPath,
                ownerPid,
    ...
    // If the original PID is alive, re-read the lock file to guard against
    // PID recycling: a different process may have taken the same PID between
    // our initial read and the process.kill check.
    if (pidAlive && ownerToken !== null) {
      try {
        const currentRaw = fs.readFileSync(lockPath, 'utf-8');
        const currentParsed = parseLockPayload(currentRaw);
        if (currentParsed.token !== ownerToken) {
          throw new ArchSpineError(
            ErrorCodes.PublishLockActive,
            "[Spine] Found a stale runtime lock (.spine/.lock) — the original owner's PID was recycled. Run 'spine build' to rebuild runtime state before publishing.",
            {
              context: {
                lockPath,
                ownerPid,
        ...
        // If re-read fails (e.g., lock deleted), treat as stale.
        throw new ArchSpineError(
          ErrorCodes.PublishLockActive,
          "[Spine] Runtime lock file changed while validating (.spine/.lock). Run 'spine build' to rebuild runtime state before publishing.",
          {
            context: {
              lockPath,
              ownerPid,
    ...
    throw new ArchSpineError(
      ErrorCodes.PublishLockActive,
      '[Spine] Runtime lock detected (.spine/.lock). Finish ongoing local sync/check/fix or clear stale lock before publishing.',
      {
        context: {
          lockPath,
  ...
  if (!exists(manifestPath) || !exists(baselinePath)) {
    throw new ArchSpineError(
      ...
      "[Spine] Runtime baseline is incomplete (.spine/manifest.json or protected-output-baseline.json missing). Run 'spine build' first.",
      {
        context: {
...
export function assertPublishSnapshotReady(rootDir: string): void {
  ...
  if (!exists(indexDir) || !exists(projectIndex)) {
    throw new ArchSpineError(
      ErrorCodes.PublishSnapshotIncomplete,
      "Distributable snapshot is incomplete (.spine/index/** missing). Re-run 'spine build'.",
      {
        context: {
...
/**
 * Emits a warning when publishing while the artifact strategy is `local`.
 *
 * In `local` mode, snapshot artifacts (.spine/index/**) are excluded from Git
...
export function warnIfPublishingFromLocalStrategy(
  ...
  runtimeIO.warn('');
  runtimeIO.warn('⚠️  [Spine] Strategy mismatch detected.');
  runtimeIO.warn(
    "   Your repository artifact strategy is set to 'local' (config: artifacts.strategy = local).",
  );
  runtimeIO.warn('   In local mode, .spine/index/** and related snapshot artifacts are');
  runtimeIO.warn('   excluded from Git by the ArchSpine-managed .gitignore block.');
  runtimeIO.warn('');
  runtimeIO.warn("   Running 'spine sync' in local mode will refresh the local snapshot,");
  runtimeIO.warn(
