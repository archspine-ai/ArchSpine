...
export async function runInfoReport(
  ...
  const llmResolution = resolveLLMSettings(config, secrets, globalConfig, globalSecrets);
  const scanner = new Scanner(rootDir, config.getScanPolicy());
  const allFiles = scanner.getAllTrackedFiles();
  const reverseIndexComplete = manifest.isReverseIndexComplete();
  const status = {
    total: allFiles.length,
    needingSync: reverseIndexComplete ? 0 : allFiles.length,
  };
