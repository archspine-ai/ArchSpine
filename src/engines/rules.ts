...
${rule.bodyMarkdown}`;
  ...
  /** Returns all currently loaded rule IDs. Used to detect stale violations from deleted rules. */
  public getAllRuleIds(): string[] {
    return this.rules.filter((r) => r.enforceable !== false).map((r) => r.ruleId);
  }
}
