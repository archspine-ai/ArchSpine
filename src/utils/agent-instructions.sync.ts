...
export type V2BlockSyncStatus =
  | 'created'
  | 'updated'
  | 'appended'
  | 'unchanged'
  | 'skipped-no-view';
export type V2BlockRemovalStatus = 'deleted' | 'updated' | 'unchanged' | 'not-found';
...
/** Verify a resolved path stays within rootDir. Throws on escape. */
function assertPathWithinRoot(rootDir: string, resolvedPath: string, context: string): void {
  ...
  if (
    ...
    throw new Error(
      `[Agent Instructions] Path traversal rejected in ${context}: "${resolvedPath}" is outside rootDir.`,
    );
  }
}

const ALLOWED_AGENT_FILENAMES: ReadonlySet<string> = new Set([
  'AGENTS.md',
  'CLAUDE.md',
  'GEMINI.md',
]);

/** Validate filename is a known agent instructions file. Rejects path traversal. */
export function normalizeAgentInstructionsFilename(filename: string): string {
  ...
  if (trimmed.includes('/') || trimmed.includes('\\') || trimmed.includes('..')) {
    // eslint-disable-next-line no-console -- CLI feedback for security-relevant input rejection
    console.warn(
      `[Agent Instructions] Filename "${trimmed}" is not a valid agent file name. Falling back to AGENTS.md.`,
    );
    return 'AGENTS.md';
  }
  if (!ALLOWED_AGENT_FILENAMES.has(trimmed)) {
    // eslint-disable-next-line no-console -- CLI feedback for security-relevant input rejection
    console.warn(
      `[Agent Instructions] "${trimmed}" is not a recognized agent file. ` +
        `Expected one of: ${[...ALLOWED_AGENT_FILENAMES].join(', ')}. Falling back to AGENTS.md.`,
    );
    return 'AGENTS.md';
  }
  return trimmed;
...
export function syncAgentInstructionsFile(
  ...
  assertPathWithinRoot(rootDir, hostPath, 'syncAgentInstructionsFile');
...
export function removeManagedAgentInstructionsFile(
  ...
  const normalizedFilename = normalizeAgentInstructionsFilename(filename);
  const hostPath = path.join(rootDir, normalizedFilename);
  assertPathWithinRoot(rootDir, hostPath, 'removeManagedAgentInstructionsFile');
  const blockPattern = getManagedBlockPattern();
...
export function syncV2BriefingBlock(
  rootDir: string,
  filename: string,
): { filePath: string; status: V2BlockSyncStatus } {
  const normalizedFilename = normalizeAgentInstructionsFilename(filename);
  const hostPath = path.join(rootDir, normalizedFilename);
  assertPathWithinRoot(rootDir, hostPath, 'syncV2BriefingBlock');

  const briefingPath = path.join(rootDir, '.spine', 'view', 'pages', 'agent-briefing.md');
  if (!fs.existsSync(briefingPath)) {
    return { filePath: hostPath, status: 'skipped-no-view' };
  }
  ...
  return { filePath: hostPath, status };
}

export function removeV2BriefingBlock(
  rootDir: string,
  filename: string,
): { filePath: string; status: V2BlockRemovalStatus } {
  const normalizedFilename = normalizeAgentInstructionsFilename(filename);
  const hostPath = path.join(rootDir, normalizedFilename);
  assertPathWithinRoot(rootDir, hostPath, 'removeV2BriefingBlock');
...
export function syncMCPToolsBlock(
  rootDir: string,
  filename: string,
): { filePath: string; status: V2BlockSyncStatus } {
  const normalizedFilename = normalizeAgentInstructionsFilename(filename);
  const hostPath = path.join(rootDir, normalizedFilename);
  assertPathWithinRoot(rootDir, hostPath, 'syncMCPToolsBlock');

  const knowledgeGraphPath = path.join(rootDir, '.spine', 'view', 'data', 'knowledge-graph.json');
...
export function removeMCPToolsBlock(
  rootDir: string,
  filename: string,
): { filePath: string; status: V2BlockRemovalStatus } {
  const normalizedFilename = normalizeAgentInstructionsFilename(filename);
  const hostPath = path.join(rootDir, normalizedFilename);
  assertPathWithinRoot(rootDir, hostPath, 'removeMCPToolsBlock');
...
export function syncHealthBlock(
  ...
  const normalizedFilename = normalizeAgentInstructionsFilename(filename);
  const hostPath = path.join(rootDir, normalizedFilename);
  assertPathWithinRoot(rootDir, hostPath, 'syncHealthBlock');

  const healthPath = path.join(rootDir, '.spine', 'view', 'pages', 'project-health.md');
...
export function removeHealthBlock(
  rootDir: string,
  filename: string,
): { filePath: string; status: V2BlockRemovalStatus } {
  const normalizedFilename = normalizeAgentInstructionsFilename(filename);
  const hostPath = path.join(rootDir, normalizedFilename);
  assertPathWithinRoot(rootDir, hostPath, 'removeHealthBlock');
