...
export class LockManager {
  private static readonly MAX_LOCK_RETRIES = 5;
  ...
  /** Acquire the runtime lock. Uses iterative retry (not recursion) to avoid stack overflow. */
  public acquire(): void {
    if (this.held) {
      return;
    }

    for (let attempt = 0; attempt < LockManager.MAX_LOCK_RETRIES; attempt++) {
      try {
        ...
        if (sysErr.code === 'EEXIST') {
          // handleExistingLock cleans a stale lock if possible, or throws
          this.handleExistingLock();
          // Stale lock was cleaned — retry the acquisition
          continue;
        }
    ...
    if (!this.held) {
      throw new ArchSpineError(
        ErrorCodes.RuntimeLockAcquireFailed,
        `[Lock] Could not acquire the runtime lock after ${LockManager.MAX_LOCK_RETRIES} attempts. ` +
          'The lock file may be persistently contested. Verify no other ArchSpine process is running, then remove .spine/.lock.',
        { context: { lockPath: this.lockPath } },
      );
    }
  ...
  private handleExistingLock(): void {
    ...
    try {
      ...
      if (sysErr.code === 'ESRCH' || sysErr.code === 'ENOENT') {
        ...
        // Stale lock cleaned — the caller's retry loop will re-attempt acquisition.
        // The OS 'wx' flag resolves any remaining race with a concurrent process.
        return;
      }
  ...
  public release(): void {
    ...
    try {
      ...
      if (this.cleanupHandler) {
        ...
        // Do not unregister 'exit' from within its own handler — it will not fire again
        // after the process exits. Only unregister when called outside exit context.
