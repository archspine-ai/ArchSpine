...
export type LLMConfigWriter = Pick<Config, 'setLLMProvider' | 'setLLMModel' | 'setLLMBaseURL'>;
...
const LLM_SET_USAGE =
  'Usage: spine llm [--global|--project] set <provider|model|base-url|api-key> <value>';
...
function handleTestCommand(runtimeService: RuntimeService): void {
  ...
  console.log(
    `LLM test passed: provider=${resolved.provider.value}, model=${resolved.model.value || 'default'}, ` +
      `apiKey=${resolved.apiKey.value ? 'configured' : 'missing'}.`,
  );
}
