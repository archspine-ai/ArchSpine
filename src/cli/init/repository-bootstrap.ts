...
export async function runRepositoryBootstrap(
  ...
  const artifactStrategy: ArtifactStrategy =
    artifactStrategyArg ||
    ((
      await prompts({
        type: 'select',
        name: 'strategy',
        message: 'Choose a repository artifact strategy:',
        choices: [
          {
            title: 'Distributable snapshot (Recommended)',
            value: 'distributable',
            description: wrapPromptText(
              'Commit generated snapshots as reviewable repository assets; keep local runtime state out of Git.',
              promptWidth,
              4,
            ),
          },
          {
            ...
            value: 'local',
            description: wrapPromptText(
              'Keep runtime files and generated snapshots out of Git for a lighter local-only setup.',
              promptWidth,
              4,
            ),
          },
