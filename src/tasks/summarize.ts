...
const VALIDATE_POLICY = resolveValidatePolicy('summarize');
...
function persistSummarizationOutputs(
  ...
  const ruleViolations = spineUnit.semantic?.ruleViolations;
  if (ruleViolations && ruleViolations.length > 0) {
    for (const v of ruleViolations) {
      ctx.manifest.recordViolation(relativeFilePath, v.id, v.severity, v.reason);
    }
  }
