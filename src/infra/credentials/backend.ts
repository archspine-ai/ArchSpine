...
export class LinuxSecretToolBackend implements CredentialBackend {
  ...
  public set(secretName: string, account: string, secret: string): void {
    try {
      ...
      // eslint-disable-next-line no-console -- User must be notified when keychain storage fails
      console.warn(
        `[Credentials] Failed to store API key via secret-tool. ` +
          `Ensure libsecret and secret-tool are installed and the keyring daemon is running. ` +
          `Falling back to file-based secrets storage.`,
      );
    }
  }
  ...
  public delete(secretName: string, account: string): void {
    try {
      ...
      // Treat missing entries as already cleared — the secret may not exist
      // or the keyring may be unavailable. Either way the state is clean.
    }
  }
}
