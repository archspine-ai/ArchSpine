...
export class ExecutionCheckpointStore {
  ...
  /** Deletes the checkpoint file and resets in-memory state. */
  public clear(): void {
    this.state = null;
  ...
  private ensureState(): ExecutionCheckpointState {
    if (!this.state) {
      // Re-load from disk before starting fresh — an in-memory null may be
      // the result of a corrupt load that set state to null, not a genuine
      // absence of a checkpoint on disk.
