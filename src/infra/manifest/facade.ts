...
export class Manifest {
  ...
  public static open(rootDir: string): Manifest {
    return new Manifest(rootDir, new SpineDB(rootDir));
  }
  ...
  public isVirginState(): boolean {
    return isVirginManifestState(this.rootDir, this.db);
  }
  ...
  public markAtlasStale(): void {
    const stalePath = path.join(this.rootDir, '.spine', '.stale');
    fs.mkdirSync(path.dirname(stalePath), { recursive: true });
    fs.writeFileSync(stalePath, `atlas-stale-at=${new Date().toISOString()}\n`, 'utf-8');
  }

  public clearAtlasStale(): void {
    const stalePath = path.join(this.rootDir, '.spine', '.stale');
  ...
  public isAtlasStale(): boolean {
    return fs.existsSync(path.join(this.rootDir, '.spine', '.stale'));
  }
}

export { hasManifestBaseline };
