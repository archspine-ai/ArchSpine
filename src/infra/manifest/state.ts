...
export function isVirginManifestState(
  rootDir: string,
  ...
  try {
    ...
    console.warn(
      `[${ErrorCodes.ManifestParseFailed}] Failed while reading ${manifestPath} during baseline check. ` +
...
export function hasManifestBaseline(rootDir: string): boolean {
  const manifestPath = getManifestPath(rootDir);
  try {
    const data = readJsonFile<Record<string, unknown>>(manifestPath) as {
      sync?: { lastSyncAt?: unknown };
    } | null;
    return typeof data?.sync?.lastSyncAt === 'string' && data.sync.lastSyncAt.trim() !== '';
  } catch {
    return false;
  }
}
