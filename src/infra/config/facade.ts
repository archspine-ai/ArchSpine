...
export class Config implements ConfigSupportedValueAccess {
  ...
  public setViewLayer(enabled?: boolean): void {
    this.saveAfter(() => {
      if (enabled === undefined) {
        this.clearArtifactValue('viewLayer');
      } else {
        this.setArtifactValue('viewLayer', enabled);
      }
    });
  }
  ...
  public getViewLayer(): boolean | undefined {
    return this.data.artifacts?.viewLayer;
  }
