...
export async function runWithFoldableOutput<T>(
  action: () => Promise<T>,
  options: { fallbackMessage?: string; label?: string } = {},
): Promise<T> {
  ...
  const animateDot = () => {
    ...
    const displayLabel = options.label || 'Syncing architecture state...';
    process.stdout.write(
      `\r${chalk.dim(displayLabel)} [${dot}] ${chalk.dim("(Press 'SPACE' for details)")}\x1b[K`,
    );
  };
