...
import { resolvePromptPolicyTier, resolveValidatePolicy } from './policy.js';
...
export function calculateSourcePromptBudgets(input: SourcePromptArtifactsInput): PromptBudgets {
  const promptTier = resolvePromptPolicyTier(input.taskMode, input.promptTier);
  ...
  const profile =
    input.taskMode === 'validate' && validatePolicy === 'strict'
      ? STRICT_VALIDATE_BUDGET_PROFILES[promptTier]
      : PROMPT_BUDGET_PROFILES[promptTier][input.taskMode];
